...
export function EditProductForm({ initialData, productId }: EditProductFormProps) {
    ...
    useEffect(() => {
        if (initialData) {
            reset({
                ...
                image_url: initialData.image_url || initialData.images || [],
                techSpecs: initialData.techSpecs || [],
    ...
    const onSubmit = async (data: Product) => {
        try {
            console.log("Submitting product update:", { id: productId, data })
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit, onInvalid)}>
            <ProductHeader isSubmitting={isPending} isFormValid={isValid} title="Ürünü Düzenle" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-8">
                    <ProductImages
                        ...
                        onRemoveImage={(index) => {
                            const currentImages = formData.image_url || []
                            setValue("image_url", currentImages.filter((_, i) => i !== index), { shouldValidate: true })
                        }}
                    />
                    ...
                    <ProductClassification
                        ...
                        badge={formData.badge as ProductBadgeType || ""}
                        onCategoryChange={(val) => setValue("category", val, { shouldValidate: true })}
                        onBadgeChange={(val) => {
                            setValue("badge", val, { shouldValidate: true })
                        }}
                    />
